...
public class ToolboxExecutor implements CommandExecutor {
    ...
    private void helpCommand(CommandSender sender) {
        if (!sender.hasPermission("terrystoolbox.help")) {
            sender.sendMessage(
                    ChatColor.DARK_RED + "Sorry! " + ChatColor.RED + "You don't have permission to use that command!");
        } else {
            ...
            sender.sendMessage(ChatColor.GOLD + "/tyatbac version: " + ChatColor.YELLOW
                    + "Shows the version of Terrance Young's Toolbox.");
            sender.sendMessage(ChatColor.GOLD + "/tyatbac update: " + ChatColor.YELLOW
                    + "Updates player data files with the current online players.");
            sender.sendMessage(ChatColor.GOLD + "/tyatbac save: " + ChatColor.YELLOW
                    + "Saves all Terry's Toolbox related data (But not dependent plugin data!).");
            sender.sendMessage(ChatColor.GOLD + "/tyatbac convert <text>: " + ChatColor.YELLOW
                    + "Tests coloured text conversion.");
        }
    }
    ...
    private void versionCommand(CommandSender sender) {
        if (!sender.hasPermission("terrystoolbox.version")) {
            ...
            sender.sendMessage(ChatColor.GOLD + "Terrance Young's Toolbox" + ChatColor.YELLOW + " - Version " + TerrysToolbox.instance.getDescription().getVersion());
        }
    }
